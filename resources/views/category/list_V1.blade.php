    ...
    <div class="container-fluid">
        ...
        <div class="row category-card-view" id="card_view">
            @foreach ($categories as $i => $category)
                <div class="col-md-3 category-item" data-bs-toggle="modal" data-bs-target="#categoryModal{{$i}}">
                    <div class="card category-card position-relative">
                        <img src="{{asset('storage/'.$category->img_path)}}" class="card-img-top" alt="{{ $category->name . 'picture' }}">
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="categoryModalLabel">Category Actions for <span class="text-success bold">{{ $category->name }}</span></h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body container">
                                    <form action="{{ route('category.destroy', $category->id) }}" method="POST" class="row text-center">
                                        @method('DELETE')
                                        @csrf
                                        <div class="col">
                                        ...
                                        <div class="col">
                                            <button type="button" class="btn btn-warning mt-3" onclick="window.location='{{ url('category/mdf/'.$category->id) }}'"><i class="bi bi-pencil-square"></i> {{__('btnText.modify')}}</button>
                                        </div>
                                        <div class="col">
                ...
                <table class="table category-table">
                    ...
                    @foreach ($categories as $j => $category)
                        <tr class="category-item">
                            <td>{{ $j+1 }}</td>
                            <td>{{ $category->name }}</td>
                            <td><img src="{{asset('storage/'.$category->img_path)}}" style="width: 100px; height: 100px;" alt="{{ $category->name . 'picture' }}" class="rounded-2"></td>
                            <td>
                                <form action="{{ route('category.destroy', $category->id) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
                                    <button type="button" class="btn btn-outline-success edit-category" onclick="window.location='{{ url('category/mdf/'.$category->id) }}'">
